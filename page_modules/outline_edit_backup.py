...
def _render_editable_outline():
    ...
    def handle_edit(chapter_data, path):
        """处理编辑操作"""
        # 根据路径找到父章节列表和索引
        parent_chapters = _find_parent_chapters_by_path(path)
        index = _get_chapter_index_by_path(path)
    ...
    def handle_delete(path):
        ...
        parent_chapters, index = _find_parent_and_index_by_path(path)
        if parent_chapters and index is not None and 0 <= index < len(parent_chapters):
...
def _update_chapter(edit_data: Dict, new_title: str, new_description: str):
    """更新章节内容"""
    # 直接更新 session_state 中的原始数据
    path = edit_data['path']
    parent_chapters = edit_data['parent_chapters']
    index = edit_data['index']
    
    # 确保索引有效
    if 0 <= index < len(parent_chapters):
        parent_chapters[index]['title'] = new_title
        parent_chapters[index]['description'] = new_description
        
        # 同时更新缓存的章节数据
        edit_data['chapter']['title'] = new_title
        edit_data['chapter']['description'] = new_description
    else:
        # 如果索引无效，尝试直接通过路径更新
        _update_chapter_by_path(path, new_title, new_description)
...
def _render_edit_dialog():
    ...
    if not st.session_state.editing_chapter:
        st.error("未找到要编辑的章节")
        if st.button("关闭", key="close_edit_no_chapter"):
            st.session_state.show_edit_dialog = False
            st.rerun()
        return
    ...
    # 显示当前章节信息
    st.markdown(f"**当前章节:** {chapter.get('id', '')}")  
    
    # 编辑表单
    with st.form(key="edit_dialog_form", clear_on_submit=False):
        st.markdown("⭐ **编辑章节信息**")
        ...
        # 预览修改
        if new_title.strip() and (new_title != chapter.get('title', '') or new_description != chapter.get('description', '')):
            st.markdown("---")
            st.markdown("🔍 **预览修改**")
            ...
            with col1:
                st.markdown("**修改前:**")
                st.info(f"标题: {chapter.get('title', '')}")  
                if chapter.get('description', ''):
                    st.info(f"描述: {chapter.get('description', '')}")
                else:
                    st.info("描述: （无）")
            
            with col2:
                st.markdown("**修改后:**")
                st.success(f"标题: {new_title.strip()}")
                if new_description.strip():
                    st.success(f"描述: {new_description.strip()}")
                else:
                    st.success("描述: （无）")
        ...
        with col1:
            if st.form_submit_button("💾 保存修改", disabled=not new_title.strip(), use_container_width=True, type="primary"):
                if new_title.strip():
                    # 更新章节数据
                    _update_chapter(edit_data, new_title.strip(), new_description.strip())
                    
                    # 清空状态
                    st.session_state.editing_chapter = None
                    st.session_state.show_edit_dialog = False
                    
                    st.success(f"章节 '{chapter.get('id', '')}' 已成功更新")
                    st.rerun()
                else:
                    st.error("章节标题不能为空")
...
def _find_parent_chapters_by_path(path: str) -> List[Dict]:
    """
    根据路径查找父章节列表
...
def _find_parent_and_index_by_path(path: str) -> Tuple[List[Dict], int]:
    """
    根据路径查找父章节列表和当前章节的索引
    ...
    try:
        ...
        return current_chapters, last_index
    except (ValueError, IndexError):
        return [], None

def _get_chapter_index_by_path(path: str) -> int:
    """
    根据路径获取章节索引
...
def _update_chapter_by_path(path: str, new_title: str, new_description: str) -> bool:
    """
    通过路径直接更新章节
        ...
        for i, part in enumerate(path_parts):
            try:
                ...
                if 0 <= index < len(current_chapters):
                    if i == len(path_parts) - 1:
                        # 到达目标章节，更新数据
                        current_chapters[index]['title'] = new_title
                        current_chapters[index]['description'] = new_description
                        return True
                    else:
                        # 继续向下搜索
                        current_chapters = current_chapters[index].get('children', [])
